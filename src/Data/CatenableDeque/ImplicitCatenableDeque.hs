...
share :: (Deque q3, Deque q1, Deque q2) => q1 a -> q2 a -> (q1 a, q3 a, q2 a)
...
instance (Deque d, Sized d) => Deque (ImplicitCatDeque d) where
  ...
  init (DEEP f a m b r)
    ...
    | not (isEmpty a) = case last a of
        SIMPLE d      -> DEEP f' (init a) m b r     where f'  = dappendR (init f) d
        CMPD f' c' r' -> DEEP f'' a'' m b r         where f'' = dappendR (init f) f'
                                                          a'' = c' ++ replaceHead (SIMPLE r') a
    | not (isEmpty b) = case last b of
        SIMPLE d      -> DEEP f' empty d (init b) r where f'  = dappendR (init f) m
        CMPD f' c' r' -> DEEP f'' a'' r' (init b) r where f'' = dappendR (init f) m
                                                          a'' = snoc c' (SIMPLE f')
...
instance (Deque d, Sized d) => CatenableDeque (ImplicitCatDeque d) where
  (SHALLOW d1)          ++ (SHALLOW d2)
    ...
    | otherwise                                  = DEEP f empty m empty r where
                                                     (f, m, r) = share d1 d2
  ...
  (DEEP f1 a1 m1 b1 r1) ++ (DEEP f2 a2 m2 b2 r2) = DEEP f1 a1' m b2' r2 where
                                                     (r1', m, f2') = share r1 f2
